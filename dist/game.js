...
class Background {
    ...
    update(height) { this.stars.forEach(s => s.update(height)); }
    draw(ctx) { this.stars.forEach(s => s.draw(ctx)); }
}
// ============================================================================
// 3. パーティクル（爆発エフェクト）
// ============================================================================
...
class Player extends GameObject {
    ...
    draw(ctx) {
        ...
        if (this.isInvincible && (this.invincibleTimer % 4 < 2))
            return;
        super.draw(ctx);
    }
}
...
class StraightBullet extends Bullet {
    constructor(x, y) { super(x, y, -Math.PI / 2, 8, '#ffff00'); }
}
...
class Item extends GameObject {
    ...
    updateWithPlayer(playerX, playerY) {
...
class GameSystem {
    constructor() {
        ...
        this.score = 0;
        this.lives = 3;
        this.weaponLevel = 1; // アイテムを取ると誘導弾が混ざる
        this.spawnTimer = 0;
        this.shotTimer = 0;
        ...
        for (const b of this.bullets) {
            for (const e of this.enemies) {
                if (b.active && e.active && this.checkCircleCollision(b, e)) {
                    ...
                    this.score += 100;
                    if (Math.random() < 0.35)
                        this.items.push(new Item(e.x, e.y)); // 35%でアイテム
                }
            }
        }
